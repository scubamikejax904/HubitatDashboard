...
private fun subgroupStatusCounts(
    tiles: List<com.tim.hubitatdash.data.model.TileConfig>,
    devices: Map<String, com.tim.hubitatdash.data.model.DeviceState>
): Pair<Int, Int> {
    var active = 0
    var inactive = 0
    for (tile in tiles) {
    ...
    if (group == null) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Column(
                ...
                Text(
                    text = "This group may have been removed or is no longer valid.",
                ...
                androidx.compose.material3.Button(
                    onClick = {
                        val fallback = resolvedGroups.firstOrNull()
                        if (fallback != null) {
                            onNavigateToGroup(fallback.id)
                        }
                    }
                ) {
                    Text("Go to a valid group")
                }
            }
        }
        return
